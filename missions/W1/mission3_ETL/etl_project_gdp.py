...
def show_gdp_over_100b(df):
    """
    GDP가 100B USD 이상인 국가를 화면에 출력한다.
    """
    gdp_over_100b = df[df["GDP_USD_billion"] >= 100]

    print("\nGDP가 100B USD 이상인 국가")
    print(gdp_over_100b.to_string(index=False))

    return gdp_over_100b


def show_region_top5_average(df):
    """
    각 Region별 GDP Top5 국가의 GDP 평균을 화면에 출력한다.
    """
    region_top5 = (
        df
    ...
    print(region_top5_avg.to_string(index=False))
...
def run_etl_pipeline():
    ...
    show_gdp_over_100b(transformed_df)
    show_region_top5_average(transformed_df)
...
if __name__ == "__main__":
    ...
    sql_result = run_query(DB_NAME, query)
