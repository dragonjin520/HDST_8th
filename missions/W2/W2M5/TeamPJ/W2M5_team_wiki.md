...
본 프로젝트의 목표는 현대자동차 관련 유튜브 댓글을 수집하고, 감정분석 모델을 적용하여 **차종별 소비자 반응 차이**를 분석하는 것이다.

단순히 댓글의 긍정/부정 여부를 판단하는 것에서 끝나는 것이 아니라, 차종별로 어떤 모델이 긍정적으로 평가되고, 어떤 모델이 부정적으로 평가되는지 비교하였다. 또한 긍정적인 차종과 부정적인 차종에서 자주 등장하는 키워드를 추출하여 소비자 반응의 원인을 해석하고자 하였다.
...
모델 학습에는 Twitter 감정분석 데이터셋을 사용하였다. 해당 데이터는 문장과 감정 라벨로 구성되어 있으며, 라벨은 다음과 같다.
...
차종은 SUV와 Sedan으로 나누어 비교하였고, 추가로 `car_name` 기준의 차종별 분석도 진행하였다.
...
팀원별 모델의 epoch별 성능을 비교한 결과, 최종 분석에는 **용선 모델**을 사용하였다.
...
모델이 예측한 감정 라벨을 기준으로 차종별 긍정/부정 비율을 비교하였다.
...
반면 가장 부정적인 반응을 보인 차종은 **Elantra**였다.

```text
Elantra 부정 비율: 43.5%
```

이를 통해 같은 현대자동차 라인업 안에서도 차종별 소비자 반응이 다르게 나타남을 확인할 수 있었다.

---

## 6. WordCloud 기반 키워드 분석

차종별 감정 비율만으로는 왜 긍정적 또는 부정적으로 평가되었는지 알기 어렵다. 따라서 가장 긍정적인 차종과 가장 부정적인 차종을 대상으로 WordCloud를 생성하여 주요 키워드를 분석하였다.

WordCloud 생성 시에는 중립 댓글을 제외하고, 긍정/부정 댓글만 사용하였다.
...
또한 더 명확한 감정 키워드를 보기 위해 threshold를 더 보수적으로 조정하였다.
...
IONIQ 9의 긍정 댓글에서는 주로 차량의 장점과 기대감을 나타내는 단어들이 등장하였다.
...
```text
IONIQ 9은 SUV 차종으로서 공간성, 디자인, 전기차 신차 기대감과 관련된 긍정 반응이 많이 나타난 것으로 해석할 수 있다.
```
...
실제 해석은 WordCloud에 나타난 주요 단어를 기준으로 구체화하면 된다.

---

## 7. 비즈니스적 해석

이번 분석을 통해 차종별 소비자 반응 차이를 정량적으로 비교할 수 있었다.
...
5. 긍정/부정 키워드를 카테고리화하여 더 구체적인 인사이트 도출
```
...
그 결과, 차종별 긍정/부정 반응 차이를 확인할 수 있었으며, IONIQ 9은 가장 긍정적인 반응을 보인 차종, Elantra는 가장 부정적인 반응을 보인 차종으로 나타났다.

또한 WordCloud를 통해 긍정 차종의 장점 키워드와 부정 차종의 단점 키워드를 추출함으로써, 감정분석 결과를 비즈니스적 의사결정에 활용할 수 있는 가능성을 확인하였다.

이번 프로젝트는 단순한 모델 학습 과제가 아니라, 실제 웹 데이터를 수집하고, 모델을 적용하고, 결과를 해석하여 차종별 소비자 반응을 분석한 데이터 파이프라인 실습이라는 점에서 의미가 있다.
